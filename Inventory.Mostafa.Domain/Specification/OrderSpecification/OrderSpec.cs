...
namespace Inventory.Mostafa.Domain.Specification.OrderSpecification
{
    public class OrderSpec:Specifications<Orders,int>
    {
        ...
        public OrderSpec(SpecParameter specParameters) : base(S => (string.IsNullOrEmpty(specParameters.Search)) || S.OrderNumber.ToLower().Contains(specParameters.Search))
        {
        ...
        public OrderSpec(OrderType orderType) : base(S => (S.OrderType.Value == orderType))
        {
            ApplyInclude();
        }
